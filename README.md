...
- [userCount](#server.set_room_properties)
...
Requests a list of all available rooms. Use a callback or register a 'get_rooms' message handler before dispatching this message.
...
Requests the current properties of the room.
...
Requests to join a specific room. If the room does not exist, it will be created.
...
Request the server to create a new room and join it automatically. The room id will be generated by the server to ensure that it is unique.
